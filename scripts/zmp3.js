...
async function out({ event, api, global, Config, logger, args }) {
  ...
  try {
    ...
    return api.sendMessage(`🔊 Có ${link.length} kết quả trùng với từ khoá tìm kiếm của bạn:\n\n${msg}\nHãy reply(phản hồi) chọn một trong những tìm kiếm trên`, event.threadID, (error, info) => global.reply.push({ name: "zmp3", messageID: info.messageID, author: event.senderID, link }), event.messageID);
  }
  catch {
